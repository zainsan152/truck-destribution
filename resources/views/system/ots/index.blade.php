            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header" style="display: flex; align-items: center;">
                        <h3 class="card-title">Liste des OTs planifiés</h3>
                    </div>
                    <div class="card-body">
...
{{--                                <th>Details</th>--}}
                            ...
                            <tbody>
                            @foreach($ots as $ot)
                                <tr id="distributionRow-{{$ot->id_ot_header }}"
                                    class="distribution-row">
                                    <td><i class="fas fa-info-circle show-distribution-details"
                                           data-distribution-id="{{$ot->id_ot_header}}"></i><span class="ml-2">{{ str_pad($ot->id_ot_header, 5, '0', STR_PAD_LEFT) }}</span></td>
                                    <td>{{$ot->client->name_client}}</td>
                                    <td>{{date('d/m/Y', strtotime($ot->execution_date))}}</td>
                                    <td>Ot{{str_pad($ot->id_ot_header, 3, '0', STR_PAD_LEFT) }}</td>
                                    <td>{{$ot->numero_ot}}</td>
                                    <td>{{date('d/m/Y', strtotime($ot->date_order))}}</td>
                                    <td>{{$ot->qty}}</td>
                                    <td>{{number_format($ot->volume)}}</td>
                                    {{--<td><i class="fas fa-info-circle show-distribution-details"
                                           data-distribution-id="{{$ot->id_ot_header}}"></i></td>--}}
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
        ...
        $(document).on('click', '.show-distribution-details', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    modalTitle.text('Details OT00' + distributionDetails.code_distribution);
